...
class SH3DImporter:
    ...
    def _create_building(self, elm):
        ...
        set_fc_type_id(building, 'building', elm.get('name'))
    ...
    def _create_ground_mesh(self, elm):
        ...
        if not ground:
            ...
            dx = bb.XLength/2
            dy = bb.YLength/2
            SW = App.Vector(bb.XMin-dx, bb.YMin-dy, 0)
            NW = App.Vector(bb.XMin-dx, bb.YMax+dy, 0)
            NE = App.Vector(bb.XMax+dx, bb.YMax+dy, 0)
            SE = App.Vector(bb.XMax+dx, bb.YMin-dy, 0)
            edge0 = Part.makeLine(SW, NW)
            edge1 = Part.makeLine(NW, NE)
            edge2 = Part.makeLine(NE, SE)
            edge3 = Part.makeLine(SE, SW)
            ground = App.ActiveDocument.addObject("Part::Feature", "Ground")
            ground.Shape = Part.makeFace([ Part.Wire([edge0, edge1, edge2, edge3]) ])
            ground.Label = "Ground"
            set_fc_type_id(ground, 'ground', 'ground')
    ...
    def _set_site_properties(self, elm):
        ...
        environment = elm.find('environment')
        ...
        bg_img = elm.find('backgroundImage')
        ...
        if compass is not None:
            ...
            self.site.Declination = self.site.northDirection
...
class BaseHandler:
    ...
    def _pv(self, v, print_z: bool = False, ndigits: int = 0):
...
class LevelHandler(BaseHandler):
    ...
    def _create_group(self, floor, prop_group_name, group_label):
        ...
        if not group:
            ...
            set_fc_property(floor, "App::PropertyString", prop_group_name, "The DocumentObjectGroup name for the group on this floor", group.Name)
    ...
    def create_slabs(self, floor, progress_bar):
        ...
        if not slab:
            ...
            if len(extrusions) > 0:
                ...
                floor.getObject(floor.SlabObjectsGroupName).addObjects(extrusions)
                ...
                set_fc_property(floor, "App::PropertyString", "ReferenceSlabName", "The name of the Slab used on this floor", None)
                return
        ...
        set_fc_type_id(slab, 'slab', f"{floor.id}-slab")
        set_fc_property(slab, "App::PropertyString", "ReferenceFloorName", "The name of the Arch.Floor this slab belongs to", floor.Name)
        set_fc_property(floor, "App::PropertyString", "ReferenceSlabName", "The name of the Slab used on this floor", slab.Name)
...
class RoomHandler(BaseHandler):
    ...
    def process(self, parent, i, elm):
        ...
        if not space:
            ...
            set_fc_property(footprint, "App::PropertyLink", "ReferenceFace", "The Reference Part.Wire", reference_face)
    ...
    def _set_properties(self, obj, elm):
        set_fc_type_id(obj, 'room', elm.get('id', str(uuid.uuid4())))
...
class BaseFurnitureHandler(BaseHandler):
    ...
    def set_piece_of_furniture_common_properties(self, obj, elm):
        ...
        set_fc_property(obj, "App::PropertyColor", "color", "The object's color", elm, default_value=self.importer.preferences["DEFAULT_FURNITURE_COLOR"])
        ...
        set_fc_property(obj, "App::PropertyString", "currency", "The object's price currency", elm, default_value='EUR')
...
class DoorOrWindowHandler(BaseFurnitureHandler):
    ...
    def process(self, parent, i, elm):
        ...
        set_fc_type_id(feature, 'doorOrWindow', door_id)
...
class FurnitureHandler(BaseFurnitureHandler):
    ...
    def process(self, parent, i, elm):
        ...
        set_fc_type_id(furniture, 'pieceOfFurniture', furniture_id)
        set_fc_property(furniture, "App::PropertyString", "ReferenceFloorName", "The Name of the Arch.Floor this walls belongs to", floor.Name)

        set_color_and_transparency(furniture, furniture.color)
...
class LightHandler(FurnitureHandler):
    ...
    def process(self, parent, i, elm):
        """Process <light> elements

        Note that a <light> element is composed of a furniture that
        is imported if requested by the user and a light source that
        is also imported if requested.

        Args:
            parent (Element): the parent of the element to be imported
            i (int): the ordinal of the imported element
            elm (Element): the xml element
        """
        ...
        if not self.importer.preferences["IMPORT_LIGHTS"]:
            return
...
class CameraHandler(BaseHandler):
    ...
    def process(self, parent, i, elm):
        ...
        set_fc_type_id(camera, 'camera', camera_id)
    ...
    def _set_properties(self, obj, elm):
        ...
        set_fc_property(obj, "App::PropertyBool", "fixedSize", "Whether the object is fixed size", bool(elm.get('fixedSize', False)))
        set_fc_property(obj, "App::PropertyEnumeration", "lens", "The object's lens (PINHOLE | NORMAL | FISHEYE | SPHERICAL)", str(elm.get('lens', "PINHOLE")), valid_values=["PINHOLE", "NORMAL", "FISHEYE", "SPHERICAL"])
